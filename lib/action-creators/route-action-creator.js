...
RouteActionCreator.prototype = {
  ...
  route: function(url, options) {
    ...
    return function(callback) {
      ...
      route.cb.call({
        t: self.translate(store.getState().locale),
        render: function(viewName, title, meta) {
          ...
          if (process.browser) {
            if (!options.isPopstate) {
              ...
              // is `false`.
              window.history.pushState(null, null, url[0] === '/' ? url : '/' + url);
            }
        ...
        route: function(redirectUrl) {
          store.dispatch(process.browser ? self.route.call(self, redirectUrl) : self.redirect(redirectUrl), callback);
        },
