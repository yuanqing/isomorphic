...
var RouteActionCreator = module.exports = function(routes, store) {
  var self = this;
  // Register our route handlers with the `Rlite` router.
  ...
  self.store = store;
  if (IS_CLIENT) {
    // Intercept link clicks if on the client-side.
    ...
    route.cb.call({
      render: function(componentName) {
        if (IS_CLIENT && !isPopstate) {
          // Only save to history if and only if routing was NOT triggered
          // by clicking the back/forward buttons.
          window.history.pushState(null, null, url);
        }
        store.dispatch(self.routeSuccess(url, componentName), resolve);
      },
      route: function(redirectUrl) {
        store.dispatch((IS_CLIENT ? self.route.bind(self) : self.redirect)(redirectUrl), resolve);
      },
      error: function(componentName) {
        store.dispatch(self.routeError(url, componentName), resolve);
      }
    }, route.params, store);
  };
};
